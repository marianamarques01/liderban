...
const headerSearchPanel = document.getElementById('header-search');
...
function setHeaderSearchOpen(isOpen) {
    ...
    const headerSearchSubmit = headerSearchPanel.querySelector('.header-search__submit');
    const tabIndex = isOpen ? '0' : '-1';

    headerSearchPanel.classList.toggle('is-open', isOpen);
    headerSearchPanel.setAttribute('aria-hidden', isOpen ? 'false' : 'true');
    headerSearchToggle.setAttribute('aria-expanded', isOpen ? 'true' : 'false');

    if (headerSearchInput) {
        headerSearchInput.tabIndex = tabIndex;
    }

    if (headerSearchSubmit) {
        headerSearchSubmit.tabIndex = tabIndex;
    }

    if (isOpen && headerSearchInput) {
        window.setTimeout(() => headerSearchInput.focus(), 150);
    }
}
...
if (headerSearchToggle && headerSearchPanel) {
    ...
    document.addEventListener('keydown', (event) => {
        if (event.key === 'Escape' && headerSearchPanel.classList.contains('is-open')) {
            setHeaderSearchOpen(false);
...
// Abre busca automaticamente na página de resultados
if (headerSearchPanel && window.location.search.includes('s=')) {
